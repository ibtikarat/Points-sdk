...
if ($dryRun) {
    ...
    echo "Would capture order {$orderUuid} against {$config['base_url']}.\n";
    exit(0);
}

$client = new Client($config);
$order = $client->orders()->capture($orderUuid);

echo "Captured order {$order->uuid} with status {$order->orderStatus->value}.\n";
